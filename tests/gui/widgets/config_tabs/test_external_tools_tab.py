...
def test_browse_repak(qtbot: Any, external_tools_tab: ExternalToolsTab) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.widgets.config_tabs.external_tools_tab.QFileDialog.getOpenFileName"
    ) as mock_dialog:
...
def test_open_url(qtbot: Any, external_tools_tab: ExternalToolsTab) -> None:
    ...
    """
    with patch(
        "foxhole_stockpiles.gui.widgets.config_tabs.external_tools_tab.QDesktopServices.openUrl"
    ) as mock_open:
