...
def prevent_dialog_on_close() -> Generator[None, None, None]:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.QMessageBox.question",
...
def configured_window(
    ...
    """
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.get_settings"
...
def test_select_pak_file(qtbot: Any, configured_window: CatalogBuilderWindow) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.QFileDialog.getOpenFileName"
    ) as mock_dialog:
...
def test_select_output_path(configured_window: CatalogBuilderWindow) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.QFileDialog.getSaveFileName"
    ) as mock_dialog:
...
def test_start_build_validation_failure(configured_window: CatalogBuilderWindow) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.QMessageBox.warning"
    ) as mock_warning:
...
def test_start_build_creates_worker(
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.CatalogBuilderWorker"
...
def test_log_key_press_event_other(configured_window: CatalogBuilderWindow) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.QTableWidget.keyPressEvent"
    ) as mock_key_press:
        event = MagicMock()
...
def test_open_settings_dialog(unconfigured_window: CatalogBuilderWindow) -> None:
    ...
    with patch(
        "foxhole_stockpiles.gui.windows.catalog_builder_window.CatalogBuilderSettingsDialog"
...
def test_open_settings_dialog_reconfigures(
    ...
    with patch(
        ...
        with patch("foxhole_stockpiles.gui.windows.catalog_builder_window.reload_settings"):
            with patch(
                "foxhole_stockpiles.gui.windows.catalog_builder_window.get_settings"
            ) as mock_get_settings:
