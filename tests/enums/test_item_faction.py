...
class TestValidateCliFaction:
    """Test suite for the validate_cli_faction static method.

    This class contains tests for CLI faction validation,
    including valid inputs and error handling.
    """

    def test_validate_cli_faction_colonials(self) -> None:
        """Test validate_cli_faction with colonials input.

        Validates that 'colonials' is accepted and returns COLONIALS.
        """
        result = ItemFaction.validate_cli_faction("colonials")
        assert result == ItemFaction.COLONIALS

    def test_validate_cli_faction_colonials_abbreviation(self) -> None:
        """Test validate_cli_faction with 'c' abbreviation.

        Validates that 'c' is accepted and returns COLONIALS.
        """
        result = ItemFaction.validate_cli_faction("c")
        assert result == ItemFaction.COLONIALS

    def test_validate_cli_faction_wardens(self) -> None:
        """Test validate_cli_faction with wardens input.

        Validates that 'wardens' is accepted and returns WARDENS.
        """
        result = ItemFaction.validate_cli_faction("wardens")
        assert result == ItemFaction.WARDENS

    def test_validate_cli_faction_wardens_abbreviation(self) -> None:
        """Test validate_cli_faction with 'w' abbreviation.

        Validates that 'w' is accepted and returns WARDENS.
        """
        result = ItemFaction.validate_cli_faction("w")
        assert result == ItemFaction.WARDENS

    def test_validate_cli_faction_case_insensitive(self) -> None:
        """Test validate_cli_faction is case insensitive.

        Validates that uppercase input is accepted.
        """
        result = ItemFaction.validate_cli_faction("COLONIALS")
        assert result == ItemFaction.COLONIALS

    def test_validate_cli_faction_invalid_exits(self) -> None:
        """Test validate_cli_faction exits with invalid input.

        Validates that invalid input causes SystemExit.
        """
        with pytest.raises(SystemExit) as exc_info:
            ItemFaction.validate_cli_faction("invalid")

        assert exc_info.value.code == 1

    def test_validate_cli_faction_none_exits(self) -> None:
        """Test validate_cli_faction exits with None input.

        Validates that None input causes SystemExit.
        """
        with pytest.raises(SystemExit) as exc_info:
            ItemFaction.validate_cli_faction(None)

        assert exc_info.value.code == 1

    def test_validate_cli_faction_empty_exits(self) -> None:
        """Test validate_cli_faction exits with empty string.

        Validates that empty string causes SystemExit.
        """
        with pytest.raises(SystemExit) as exc_info:
            ItemFaction.validate_cli_faction("")

        assert exc_info.value.code == 1

    def test_validate_cli_faction_neutral_exits(self) -> None:
        """Test validate_cli_faction exits with neutral input.

        Validates that explicit 'neutral' input causes SystemExit
        since only colonials or wardens are allowed for CLI filtering.
        """
        with pytest.raises(SystemExit) as exc_info:
            ItemFaction.validate_cli_faction("neutral")

        assert exc_info.value.code == 1

    def test_validate_cli_faction_error_message(self, capsys: pytest.CaptureFixture[str]) -> None:
        """Test validate_cli_faction error message output.

        Validates that error messages are printed to stdout.
        """
