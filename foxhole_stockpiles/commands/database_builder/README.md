...
The `database_builder.py` tool processes icon templates generated by `generate_training_images.py` and creates an optimized binary database with pre-computed features for template matching. It enables the multi-stage filtering approach required to achieve 1-4ms recognition performance.
...
2. **Template Generation** (`generate_training_images.py`) - Creates resolution-specific templates
3. **Database Building** (`database_builder.py`) - **THIS TOOL** - Builds optimized binary database
...
```bash
python database_builder.py --catalog CATALOG --templates TEMPLATES --output OUTPUT [OPTIONS]
```
...
python database_builder.py \
    ...
    --templates ../../training_images/ \
    --output ../../data/icons.fhic
```
...
The input should be the output from `generate_training_images.py`:
